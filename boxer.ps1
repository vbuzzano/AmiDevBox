...
.NOTES
    Build Date: 2026-01-21 00:57:53
    Version: 0.1.204
    Build Type: Embedded
#>
...
#
# NOTE: This file should contain ONLY functions, no executable code
# All initialization is done by calling Initialize-Boxing from the main scripts
...
function Import-CoreLibraries {
    ...
    $coreFiles = Get-ChildItem -Path $corePath -Filter '*.ps1' |
        Where-Object { $_.Name -notin @('bootstrapper.ps1', 'module-loader.ps1', 'dispatcher.ps1', 'metadata.ps1', 'system-install.ps1') } |
        Sort-Object Name
...
function Get-InstalledVersion {
    <#
    .SYNOPSIS
    Gets the version from an installed script file.

    .DESCRIPTION
    Reads a PowerShell script file and extracts the version string
    from a $script:BoxerVersion variable assignment.

    .PARAMETER ScriptPath
    Path to the script file (boxer.ps1, box.ps1, etc.)

    .OUTPUTS
    Version string (e.g., "1.0.0") or $null if not found

    .EXAMPLE
    Get-InstalledVersion -ScriptPath "$env:USERPROFILE\Documents\PowerShell\Boxing\boxer.ps1"
    #>
    param(
        [Parameter(Mandatory=$true)]
        [string]$ScriptPath
...
function Compare-Version {
    ...
    .DESCRIPTION
    Compares two semantic version strings using .NET version comparison.
    ...
    .EXAMPLE
    Compare-Version -Version1 "1.0.0" -Version2 "1.0.1"
    Returns -1 (1.0.0 is older than 1.0.1)
    #>
    param(
        [Parameter(Mandatory=$true)]
        [string]$Version1,
...
function Install-Box {
    <#
    .SYNOPSIS
    Installs or updates a box from GitHub URL or simple name.

    .PARAMETER BoxUrl
    GitHub repository URL or simple box name (e.g., "AmiDevBox").

    .PARAMETER Force
    Force reinstallation even if box is already installed.
    ...
    try {
        ...
        $InstalledVersion = Get-InstalledVersion -ScriptPath $BoxScriptPath

        # Get remote version from GitHub
        $RemoteVersion = $null
        try {
            $MetadataUrl = "https://github.com/$Owner/$Repo/raw/main/metadata.psd1"
            $RemoteMetadataContent = Invoke-RestMethod -Uri $MetadataUrl -ErrorAction Stop

            if ($RemoteMetadataContent -match 'Version\s*=\s*"([^"]+)"') {
                $RemoteVersion = $Matches[1]
            }
        } catch {
            Write-Verbose "Could not fetch remote version, proceeding with install"
        }

        # Determine if update/install is needed
        $NeedsUpdate = $false
        $UpdateReason = ""

        if (-not (Test-Path $BoxDir)) {
            $NeedsUpdate = $true
            $UpdateReason = "Installing $BoxName..."
            Write-Step $UpdateReason
        } elseif ($Force) {
            $NeedsUpdate = $true
            $UpdateReason = "Reinstalling $BoxName..."
            Write-Step $UpdateReason
        } elseif ($RemoteVersion -and $InstalledVersion -and (Compare-Version -Version1 $RemoteVersion -Version2 $InstalledVersion) -gt 0) {
            $NeedsUpdate = $true
            $UpdateReason = "Updating $BoxName ($InstalledVersion → $RemoteVersion)..."
            Write-Step $UpdateReason
        ...
        $BoxScriptUrl = "https://github.com/$Owner/$Repo/raw/main/box.ps1"
        try {
            Invoke-RestMethod -Uri $BoxScriptUrl -OutFile $BoxScriptPath
            Write-Success "Downloaded: box.ps1"
        } catch {
            throw "Failed to download box.ps1: $_"
        }

        # Download config.psd1 (optional)
        ...
        # Download metadata.psd1 (optional)
        Write-Step "Downloading metadata.psd1..."
        $MetadataUrl = "https://github.com/$Owner/$Repo/raw/main/metadata.psd1"
        try {
            Invoke-RestMethod -Uri $MetadataUrl -OutFile $BoxMetadataPath
            Write-Success "Downloaded: metadata.psd1"
        } catch {
            Write-Host "  Warning: metadata.psd1 not found (optional)" -ForegroundColor Yellow
        }

        # Download env.ps1 (optional)
        Write-Step "Downloading env.ps1..."
        $EnvUrl = "https://github.com/$Owner/$Repo/raw/main/env.ps1"
        $EnvPath = Join-Path $BoxDir "env.ps1"
        try {
            Invoke-RestMethod -Uri $EnvUrl -OutFile $EnvPath
            Write-Success "Downloaded: env.ps1"
        } catch {
            Write-Host "  Warning: env.ps1 not found (optional)" -ForegroundColor Yellow
        }
        ...
        try {
            ...
            foreach ($File in $TplFiles) {
                # Download ONLY files at root of tpl/, skip directories
